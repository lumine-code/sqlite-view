...
class BrowseClient {
  ...
  request(op, payload = {}, { replaceKey = null } = {}) {
    if (this.destroyed) return Promise.reject(codeError("DESTROYED"));
    return new Promise((resolve, reject) => {
      if (replaceKey) {
        const previous = this.queue.findIndex((entry) => entry.replaceKey === replaceKey);
        if (previous >= 0) {
          this.queue.splice(previous, 1)[0].reject(codeError("SUPERSEDED"));
        }
      }
      this.queue.push({ op, payload, resolve, reject, replaceKey });
      this.pump();
    });
  }
  ...
  pump() {
    ...
    this.watchdog = setTimeout(() => {
      ...
      this.restart(this.filePath, { invalidate: true });
    }, BROWSE_WATCHDOG_MS);
    try {
  ...
  cancelQueued(replaceKey) {
    const index = this.queue.findIndex((entry) => entry.replaceKey === replaceKey);
    if (index < 0) return false;
    this.queue.splice(index, 1)[0].reject(codeError("SUPERSEDED"));
    return true;
  }
  ...
  restart(filePath = this.filePath, { invalidate = false } = {}) {
    this.filePath = filePath;
    this.stopBrowseTask();
    ...
    this.pump();
    if (invalidate) {
      this.emitter.emit("did-change-database", {
        restarted: true,
        revision: this.revision,
      });
    }
