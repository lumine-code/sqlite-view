...
class CanvasGrid {
  constructor(options = {}) {
    if (!options.document?.defaultView) {
      throw new TypeError("CanvasGrid requires a live Document");
    }
    ...
    this.disposables = [];
    this.requestFrame = options.requestAnimationFrame || null;
    this.cancelFrame = options.cancelAnimationFrame || null;
    this.getStyle =
      options.getComputedStyle ||
      ((element) => element.ownerDocument.defaultView.getComputedStyle(element));
    this.getDpr = options.getDevicePixelRatio || (() => this.getWindow().devicePixelRatio || 1);
    this.clipboard =
    ...
    this.buildElement(options.document);
  ...
  observeResize(factory) {
    const create = factory || ((callback) => new (this.getWindow().ResizeObserver)(callback));
  ...
  prepareSurfaceTransition() {
    this.stopPointerInteractions();
    this.resizeObserver?.disconnect?.();
    if (this.frameHandle != null) {
      try {
        if (this.cancelFrame) this.cancelFrame(this.frameHandle);
  ...
  commitSurfaceTransition() {
    this.observeResize(this.options.resizeObserverFactory);
    this.readTheme();
    this.computeLayout();
    this.resize();
    // Transition commit precedes the native show; do not expose the canvas
    // with the source realm's DPI/theme for one frame.
    this.draw();
    this.scheduleDraw();
  }
  ...
  stopPointerInteractions() {
    ...
    this.resizeState = null;
    try {
      this.dragWindow?.removeEventListener("mousemove", this.handlers.windowMousemove);
      this.dragWindow?.removeEventListener("mouseup", this.handlers.windowMouseup);
    } catch {
      // Recovery can begin after the owning native Window has closed.
    }
    try {
      this.resizeWindow?.removeEventListener("mousemove", this.handlers.resizeMousemove);
      this.resizeWindow?.removeEventListener("mouseup", this.handlers.resizeMouseup);
    } catch {
      // Recovery can begin after the owning native Window has closed.
    }
    this.dragWindow = null;
  ...
  scheduleDraw() {
    ...
    this.frameWindow = this.getWindow();
    const requestFrame =
      this.requestFrame || this.frameWindow.requestAnimationFrame.bind(this.frameWindow);
  ...
  handleMouseDown(event) {
    ...
    if (resize) {
      ...
      this.resizeWindow.addEventListener("mousemove", this.handlers.resizeMousemove);
      this.resizeWindow.addEventListener("mouseup", this.handlers.resizeMouseup);
      event.preventDefault();
      return;
    }
    ...
    this.dragging = true;
    this.dragWindow = this.getWindow();
    this.dragWindow.addEventListener("mousemove", this.handlers.windowMousemove);
    this.dragWindow.addEventListener("mouseup", this.handlers.windowMouseup);
    event.preventDefault();
  }
  ...
  destroy() {
    ...
    if (this.frameHandle != null) {
      try {
        ...
        // The owning native Window may already be closed.
      }
    }
    this.resizeObserver?.disconnect?.();
