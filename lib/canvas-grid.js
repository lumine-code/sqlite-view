...
class CanvasGrid {
  ...
  readTheme() {
    ...
    if (previousFont && previousFont !== this.font) this.autoColumnWidths = [];
    ...
    this.colorBackground = value("--pane-item-background-color", "#1e1e1e");
  ...
  fitText(text, maximumWidth) {
    ...
    if (this.ctx.measureText(text).width <= maximumWidth) return text;
    const ellipsis = "…";
    let low = 0;
