...
const GRID_TEXT_BYTES = GRID_TEXT_LIMIT * 4 + 4;
...
function safeScalarProjection(expression) {
  return [
    ...
    `CASE typeof(${expression})
       WHEN 'text' THEN CASE WHEN length(CAST(${expression} AS BLOB)) <= ${CURSOR_VALUE_LIMIT}
...
function gridProjection(expression) {
  return [
    `typeof(${expression})`,
    `CASE typeof(${expression})
       WHEN 'text' THEN CAST(substr(CAST(${expression} AS BLOB), 1, ${GRID_TEXT_BYTES}) AS TEXT)
       WHEN 'blob' THEN length(${expression})
       ELSE ${expression}
     END`,
    `CASE typeof(${expression})
       WHEN 'text' THEN CASE WHEN length(CAST(${expression} AS BLOB)) <= ${GRID_TEXT_BYTES}
                             THEN 0 ELSE 1 END
...
class BrowseEngine {
  ...
  page(request) {
    ...
    if (!this.planCache.has(planKey)) {
      if (this.planCache.size >= MAX_PLAN_CACHE_ENTRIES) {
  ...
  cell(request) {
    ...
    const sql = `SELECT typeof(${expression}),
                        CASE typeof(${expression})
                          WHEN 'text' THEN CAST(substr(CAST(${expression} AS BLOB), 1, ${CELL_TEXT_LIMIT}) AS TEXT)
                          WHEN 'blob' THEN substr(${expression}, 1, ${CELL_BLOB_LIMIT})
                          ELSE ${expression}
                        END,
                        CASE typeof(${expression})
                          WHEN 'text' THEN CASE WHEN length(CAST(${expression} AS BLOB)) <= ${CELL_TEXT_LIMIT} THEN 0 ELSE 1 END
                          WHEN 'blob' THEN length(${expression})
                          ELSE NULL
                        END
