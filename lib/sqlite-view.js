...
class SQLiteView {
  constructor(filePath, state = {}) {
    ...
    this.fileState = fs.existsSync(this.filePath) ? FileState.UNMODIFIED : FileState.REMOVED;
  ...
  redirectFocus = (event) => {
    ...
    this.cancelRedirectFocusFrame();
    const document = this._element.ownerDocument;
    this.redirectFocusWindow = document.defaultView;
    this.redirectFocusFrame = this.redirectFocusWindow.requestAnimationFrame(() => {
      this.redirectFocusFrame = null;
      this.redirectFocusWindow = null;
      if (document.activeElement === this._element) this.focus();
    });
  };

  cancelRedirectFocusFrame() {
    if (this.redirectFocusFrame != null) {
      try {
        this.redirectFocusWindow?.cancelAnimationFrame(this.redirectFocusFrame);
      } catch {
  ...
  async beginWindowSurfaceTransition() {
    this.cancelRedirectFocusFrame();
    return this.component?.beginWindowSurfaceTransition() || null;
  }
