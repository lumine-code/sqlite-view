...
class SQLiteView {
  constructor(filePath, state = {}) {
    ...
    this.armFileWatcher();
    if (!fs.existsSync(this.filePath)) this.handleDelete();
  }
  ...
  armFileWatcher() {
    ...
    this.fileSubscriptions.add(
      ...
      file.onDidError((error) => console.error("Unable to watch database", error)),
    );
  ...
  setPath(nextPath) {
    ...
    this.armFileWatcher();
    this.component?.handleFileRenamed(nextPath);
  ...
  reconcileFile() {
    ...
    if (!fs.existsSync(this.filePath)) return this.handleDelete();
    const wasMissing = this.fileMissing;
    this.fileMissing = false;
    this.setFileState(FileState.UNMODIFIED);
    if (wasMissing) this.component?.handleFileAvailable();
    else this.component?.handleExternalChange();
  }
